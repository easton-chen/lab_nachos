...
char* const strThreadStatus[] = {"JUST_CREATED", "RUNNING", "READY", "BLOCKED"};
...
//----------------------------------------------------------------------
// ShowAllThreadStatus, threadPrintStatus
//  command TS will call this function, to display all the threads 
//  and their status. add in lab1 ex4.
//----------------------------------------------------------------------

void threadPrintStatus(int arg)
{
    Thread *t = (Thread *)arg;
    printf("%-20s \t\t %-10d \t\t %-10s\n", t->getName(), t->getThreadID(),
         strThreadStatus[t->getStatus()]);
}

void ShowAllThreadStatus()
{
    printf("thread name           \t\t threadID   \t\t status    \n");
    List *readyThreadList = scheduler->getReadyList();
    readyThreadList->Mapcar((VoidFunctionPtr) threadPrintStatus);
    printf("%-20s \t\t %-10d \t\t %-10s\n", currentThread->getName(), currentThread->getThreadID(),
         strThreadStatus[currentThread->getStatus()]);
}
